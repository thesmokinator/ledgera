...
function App() {
  ...
  const [pendingRecurringInfo, setPendingRecurringInfo] = useState<{
    count: number;
    rules: number;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    const dismissedMonth = localStorage.getItem("recurring_dismiss_month");
    const currentMonth = dayjs().format("YYYY-MM");
    if (dismissedMonth === currentMonth) return;

    callCommand<PendingRecurringDates[]>("compute_pending_recurring")
      .then((result) => {
        const totalPending = result.reduce((sum, r) => sum + r.dates.length, 0);
        if (totalPending > 0) {
          setPendingRecurringInfo({
            count: totalPending,
            rules: result.length,
          });
      ...
      .catch(() => {});
  }, [activeSettings.journalPath, activeSettings.modules.autoGenerateRecurring]);
  ...
  const navigationItems = useMemo<NavigationItem[]>(
    () => {
      ...
      if (activeSettings.modules.gitSync.enabled) {
        ...
        if (gitSyncStatus && summary.tone !== "success" && summary.tone !== "neutral") {
          if (summary.tone === "danger") {
            syncBadge = t("sync.nav_badge_issue");
          } else if (summary.labelKey === "sync.status_changes") {
            syncBadge = <WarningOutlined style={{ fontSize: 12, display: "flex" }} />;
          } else {
            syncBadge = t(summary.labelKey, summary.labelOptions);
          }
        }
    ...
    >
      <Layout className={`app-shell ${isDarkTheme ? "theme-dark" : "theme-light"}${isMacOs ? " platform-macos" : ""}`}>
        ...
        <Modal
          ...
          title={t("recurring.auto_generate_title")}
          onCancel={() => {
            if (dontAskAgain) {
              localStorage.setItem("recurring_dismiss_month", dayjs().format("YYYY-MM"));
            }
          ...
          footer={[
            <Button key="later" onClick={() => {
              ...
              {t("recurring.auto_generate_later")}
            </Button>,
            <Button key="generate" type="primary" onClick={() => {
              ...
              callCommand("generate_recurring_transactions", { ruleIdFilter: null })
                .then(() => {
                  ...
                  messageApi.success(
                    t("recurring.generated_count_plural", { count: pendingRecurringInfo?.count ?? 0 })
                  );
                })
                .catch(() => {});
            }}>
              {t("recurring.generate_all")}
            </Button>,
          ]}
        >
          <p>{t("recurring.auto_generate_message", { count: pendingRecurringInfo?.count ?? 0, rules: pendingRecurringInfo?.rules ?? 0 })}</p>
          <Checkbox checked={dontAskAgain} onChange={(e) => setDontAskAgain(e.target.checked)}>
            {t("recurring.auto_generate_dont_ask")}
          </Checkbox>
        </Modal>
      </Layout>
    </ConfigProvider>
